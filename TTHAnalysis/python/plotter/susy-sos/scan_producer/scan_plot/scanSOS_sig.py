...
class Scan(object):
    ...
    def loadData(self):
        if self.name == 'T2tt':
            ...
            self.xbinsize = 1. 
            self.ybinsize = 1.
            self.xbins = binning(300,530, self.xbinsize)
            self.ybins = binning(  7,100, self.ybinsize)
            ...
            self.zminUL = -3; self.zmaxUL = 3
            self.zmaxEff = 0.30
            self.xsecFile = 'xs_T2tt.root'
            self.xtitle = 'm_{stop}'; self.ytitle = 'm_{LSP}'
            self.limitfile = 'limits/significance_T2tt_Moriond_FINAL.root'

        if self.name == 'TChiNeuWZ':
            self.makeMCDatacards = True
            self.paper = 'SUS16025'
            self.xbinsize = 10. #1. #10.
            self.ybinsize = 4.  #0.5 # 4.
            self.xbins = binning(95,295, self.xbinsize)
            ...
            self.limitfile = 'limits/significance_TChiWZ_Moriond_FINAL.root'#'limits/scan_TChiWZ_Moriond_approval.root'#'limits/scan_TChiWZ_Moriond_v1.root'#'limits/scan_TChiWZ_noPU_xsTH_scale.root'#'scan_TChiWZ_v2.root'
    ...
    def makeExclusion(self):
        ...
        self.ex_obs    .GetZaxis().SetRangeUser(-3.,3.)
    ...
    def makeULPlot(self):
        ...
        for i in range(h_rhisto.GetNbinsX()+1):
            for j in range(h_rhisto.GetNbinsY()+1):
                xs = 1.0 #self.xsec_histo.GetBinContent( self.xsec_histo.FindBin(int(h_rhisto.GetXaxis().GetBinCenter(i))))                                
    ...
    def saveULGraphsInFile(self):
        ...
        f = r.TFile('makeExclusionPlot/config/%s/%s_significance_results.root'%(self.paper,self.name),'RECREATE')
        f.cd()        
        self.ul_histo.Write()
